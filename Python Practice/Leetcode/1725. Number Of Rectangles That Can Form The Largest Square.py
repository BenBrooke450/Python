...
You can cut the ith rectangle to form a square with
    ...
    For example, if you have a rectangle [4,6], you
...
def countGoodRectangles(rectangles: list[list[int]]):
    a = [min(n) for n in rectangles]
    return a.count(max([min(n) for n in rectangles]))
