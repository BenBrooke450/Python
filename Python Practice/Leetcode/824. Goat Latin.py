...
You are given a string sentence that consist of words separated
    by spaces. Each word consists of lowercase and uppercase letters only.
...
def toGoatLatin(sentence: str) -> str:
    ...
    for i,word in enumerate(sentence):
        if word[0] in vowel:
            new_sent.append(word + "ma" + "a"*(i+1))
        elif word[0] not in vowel:
            new_sent.append(word[1:] + word[0] + "a"*(i+1))

    return " ".join(new_sent)


print(toGoatLatin("I speak Goat Latin"))
# Imaa peaksaa oatGaaa atinLaaaa
