...
The population of some year x is the number of people alive during
    that year. The ith person is counted in year x's population
...
def maximumPopulation(logs: list[list[int]]) -> int:
    ...
    max_date = max(max(logs))
    min_date = min(min(logs))
    list1 = []
    for year in range(min_date,max_date):
        n = 0
        for life in logs:
            if year in range(life[0],life[1]):
                n = n + 1
        list1.append([year,n])
...
print(maximumPopulation([[2000,2001]]))
