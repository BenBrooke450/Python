...
Return an integer array answer where answer[0] is the number of
    candies in the box that Alice must exchange, and answer[1]
...
def fairCandySwap(aliceSizes: list[int], bobSizes: list[int]) -> list[int]:
    s_a = sum(aliceSizes)
    s_b = sum(bobSizes)
    for j, m in enumerate(aliceSizes):
        diff = abs((s_a - aliceSizes[j]) - (s_b + aliceSizes[j])) / 2
        if diff % 1 == 0 and diff in bobSizes:
            i = bobSizes.index(diff)
            if bobSizes[i] - aliceSizes[j] + s_a == aliceSizes[j] + s_b - bobSizes[i]:
                return [aliceSizes[j], bobSizes[i]]

print(fairCandySwap(aliceSizes = [1,1], bobSizes = [2,2]))

print(fairCandySwap(aliceSizes = [1,1], bobSizes = [2,2]))

print(fairCandySwap(aliceSizes = [2], bobSizes = [1,3]))
...
def fairCandySwap(aliceSizes: list[int], bobSizes: list[int]) -> list[int]:
    ...
    print(abs(s_a-s_b))
    for j, m in enumerate(aliceSizes):
        for i, n in enumerate(bobSizes):
            if (s_a - aliceSizes[j] + bobSizes[i]) == (s_b - bobSizes[i] + aliceSizes[j]):
                return [aliceSizes[j], bobSizes[i]]
