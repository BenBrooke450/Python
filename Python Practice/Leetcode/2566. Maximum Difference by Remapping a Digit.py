...
Return the difference between the maximum and minimum
    values Bob can make by remapping exactly one digit in num.
...
def minMaxDifference(num: int) -> int:
    max_list = [x for x in str(num)]

    for i,n in enumerate(max_list):
        if max_list[i] == "9":
            pass
            if i +1 == len(max_list):
                max_value = int("".join(max_list))
        elif max_list[i] != "9":
            max_value = ("".join(max_list)).replace(n,"9")
    ...
    for n in range(9+1):
        n = str(n)
        list1.append(int("".join(max_list).replace(n,"0")))
    min_value = min(list1)
