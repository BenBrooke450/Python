...
def splitNum(num: int) -> int:
    sorted_num = sorted([x for x in str(num)])
    list1 = []
    list2 = []
    n = 0
    ...
    while n < len(str(num)):
        if n % 2 == 0:
            list1.append(sorted_num[0])
            sorted_num = sorted_num[1:]
            n = n + 1

        if n % 1 == 0:
            list2.append(sorted_num[0])
            sorted_num = sorted_num[1:]
            n = n + 1
...
def splitNum(num: int) -> int:
    ...
    n = 0
    if len(sorted_num) == 2:
        return sum(int(x) for x in sorted_num)

    while n != len(str(sorted_num)):
        list1.append(int(sorted_num[0] + sorted_num[-1]))
        sorted_num = sorted_num[1:-1]
        if len(sorted_num) == 2 or len(sorted_num) == 1 :
            x = int("".join(sorted_num))
            list1.append(x)
            return sum(list1)
        elif len(sorted_num) == 0:
            return sum(list1)
