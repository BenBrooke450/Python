...
If Teemo attacks again before the poison effect ends, the timer for
it is reset, and the poison effect will end duration seconds after the new attack.
...
def teemo(list1:list[int],number:int):
    list2 = []
    for x in list1:
        list2.extend(list(range(x,x+number)))
