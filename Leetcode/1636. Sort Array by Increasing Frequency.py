...
def frequencySort(nums: list[int]):
    ...
    sorted_nums = sorted(nums, key = lambda x: (dict1.get(x),x))

    sorted_nums_within = sorted(sorted_nums, key=lambda x: dict1.get(x) == x )

    print(sorted_nums_within)
...
#sorted_data = sorted(data, key=lambda x: x['age'] if x['age'] > 21 else float('inf'))
